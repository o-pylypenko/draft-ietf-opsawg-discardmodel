...
This document is mainly scoped to reporting packet loss at Layer 3, frames discarded at Layer 2, and in between (e.g., MPLS {{?RFC3031}}). This document considers only the signals that may trigger automated mitigation actions and not how the actions are defined or executed. These considerations are deployment-specific. Likewise, the document does not correlate specific discard counters with other events and counters (e.g., ICMP). Such a correlation is not trivial and depends on local configuration and specific conditions (e.g., rate-limit policy, overload, or Access Control Lists (ACLs)).
